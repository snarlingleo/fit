...
const Gamification = {
  ...
  TROPHEES_DEF: [
    ...
    { id:'first_session',  nom:'Première séance',    emoji:'🎯', xp:100,
      description:'Terminer sa première séance',     condition: d => d.totalSeances >= 1 },
    { id:'first_pr',       nom:'Premier record',     emoji:'🏆', xp:150,
      description:'Battre un premier PR',            condition: d => d.totalPRs >= 1 },
    { id:'first_week',     nom:'Première semaine',   emoji:'📅', xp:200,
      description:'Compléter une semaine entière',   condition: d => d.seancesParSemaine >= 4 },
    ...
    { id:'sessions_10',    nom:'10 séances',         emoji:'💪', xp:150,
      description:'Atteindre 10 séances totales',   condition: d => d.totalSeances >= 10 },
    { id:'sessions_25',    nom:'25 séances',         emoji:'🏋️', xp:300,
      description:'Atteindre 25 séances',           condition: d => d.totalSeances >= 25 },
    { id:'sessions_50',    nom:'50 séances',         emoji:'🎖️', xp:500,
      description:'Atteindre 50 séances',           condition: d => d.totalSeances >= 50 },
    { id:'sessions_100',   nom:'Centurion',          emoji:'💯', xp:1000,
      description:'Atteindre 100 séances',          condition: d => d.totalSeances >= 100 },
    { id:'sessions_200',   nom:'Légende vivante',    emoji:'👑', xp:2000,
      description:'Atteindre 200 séances',          condition: d => d.totalSeances >= 200 },

    // PRs
    { id:'prs_5',          nom:'Collectionneur',     emoji:'🏅', xp:200,
      description:'Battre 5 records différents',    condition: d => d.totalPRs >= 5 },
    { id:'prs_10',         nom:'Record Man',         emoji:'🎯', xp:400,
      description:'Battre 10 records',              condition: d => d.totalPRs >= 10 },
    { id:'prs_20',         nom:'Maître des records', emoji:'👑', xp:800,
      description:'Battre 20 records',              condition: d => d.totalPRs >= 20 },
    ...
    // Assiduité
    { id:'morning_5',      nom:'Lève-tôt',           emoji:'🌅', xp:150,
      description:'5 séances avant 8h',
      condition: d => d.seancesMatin >= 5 },
    { id:'perfect_month',  nom:'Mois parfait',       emoji:'🌟', xp:800,
      description:'4+ séances/semaine pendant 4 semaines',
      condition: d => d.moisParfait },
  ...
  getXP() {
    ...
    const xpRange = niveau.xpSuivant - niveau.xpMin;
    const pct     = Math.min(100, Math.round((xpNiv / xpRange) * 100));
  ...
  ajouterXP(montant, raison = '') {
    ...
    if (nivApres.numero > nivAvant.numero) {
      ...
      Utils.toast(`🎉 NIVEAU ${nivApres.numero} — ${nivApres.emoji} ${nivApres.nom} !`, 'success', 5000);
    } else if (raison) {
      Utils.toast(`+${montant} XP — ${raison}`, 'info', 2000);
    }

    console.log(`[XP] +${montant} (${raison}) → Total: ${apres}`);
    return apres;
  },
  ...
  verifierTrophees() {
    ...
    const donneees = {
      ...
      prs,
      seancesMatin:      Utils.storage.get('ft_seances_matin', 0),
      moisParfait:       Utils.storage.get('ft_mois_parfait', false),
    ...
    if (nouveaux.length > 0) {
      ...
      nouveaux.forEach(t => {
        setTimeout(() => {
          ...
          Utils.toast(`🏆 Trophée débloqué : ${t.emoji} ${t.nom} — +${t.xp} XP`, 'pr', 5000);
  ...
  recompenser(action, contexte = {}) {
    const montant = this.XP_ACTIONS[action] || 0;
    if (montant > 0) {
      this.ajouterXP(montant, action.toLowerCase().replace('_',' '));
    }
...
console.log('✅ Gamification chargé');
